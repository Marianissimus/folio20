<template>
  <div id="nav">
    ...
    <div class="nav-name nav-element">
      ...
      >
        <span @click="scrollTo(top)" :style="{'transition-delay': '2000ms'}">Marian Vidoe</span>
      </transition>
    </div>
    <div class="nav-links">
      <transition
      ...
      >
        <span @click="scrollTo(link.element)" :style="{'transition-delay': 2000 + index * 400  + 'ms'}">{{ link.text }}</span>
      </transition>
    </div>
    <div class="options">
      <transition
        ...
        appear-active-class="link-appear-active-class"
      >
      <div class="selector-icon" @click.stop="showThemeSelector = !showThemeSelector" :style="{'transition-delay': '3400ms' }">
    ...
    scrollTo (where) {
      if (this.$route.name === 'Projects') {
        this.$router.push('/').then(() =>
          setTimeout(() => document.getElementById(where).scrollIntoView({behavior: "smooth"}), 100)
        )
      }
      if (where === 'top') {
        window.scrollTo({
